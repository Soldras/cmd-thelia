...
\n
Usage: D_customer.sh  [-h] [-o] [-c] [-z] -d database -u user -p password -f filename\n
\n
Options:\n
...
"
while getopts "oczhd:u:p:f:" option
do
	case $option in
		o)
			...
			;;
		c)
			TABLES=$TABLES$TABLES_CART
			;;
		z)
			TABLES=$TABLES$TABLES_OLD
			;;
		d)
		    DB="$OPTARG"
			;;
